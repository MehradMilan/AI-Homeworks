...
class QLearner(rl_agent.AbstractAgent):
    ...
    def _epsilon_greedy(self, info_state, legal_actions, epsilon):
        ...
        best_q = max([self._q_values[info_state][a] for a in legal_actions])
        action = np.argmax([self._q_values[info_state][action] for action in legal_actions])
        greedy_actions = [
            action for action in legal_actions if self._q_values[info_state][action] >= best_q
        ]
    ...
    def find_reward(self, time_step, with_mask=False):
    ...
    def step(self, time_step, is_evaluation=False, top1=False):
        ...
        if self._prev_info_state and not is_evaluation:
            reward = self.find_reward(time_step, True)
            ...
            self._last_loss_value =  np.square(self._q_values[self._prev_info_state][self._prev_action] - target)
